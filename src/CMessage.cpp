...
#include <string.h>
CMessage* CMessage::instance = NULL;
CMessage::CMessage()
{
    memset(g_MsgDrvObj.msgTab, 0, sizeof(MSGTAB_Class) * MAX_MSGID_NUM);
}
...
void CMessage::MSGDRIV_register(int msgId,MsgApiFun pRtnFun, void* context)
{
    assert(msgId < MAX_MSGID_NUM);
...
void* CMessage::MSGDRIV_ProcTask()
{
    ...
    while (pMsgDrvObj->tskLoop)
    {
        ...
        if ((iret != SDK_EFAIL) && (msg_recv.cmd != MSGDRIV_FREE_CMD))
        {
            ...
            SDK_ASSERT(msg.msgId >= 0 && msg.msgId < MAX_MSGID_NUM);
